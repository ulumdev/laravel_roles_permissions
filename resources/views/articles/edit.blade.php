                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('articles.update', $article->id) }}" method="POST">
                        ...
                        <div>
                            ...
                            <div class="my-3">
                                <input value="{{ old('title', $article->title) }}" type="text" id="title"
                                    name="title" class="form-input border-gray-300 shadow-sm w-1/2 rounded-lg"
                                    placeholder="Title">
                                @error('title')
                            ...
                            <div class="my-3">
                                <textarea id="content" name="content" class="form-input border-gray-300 shadow-sm w-1/2 rounded-lg" cols="30"
                                    rows="5" placeholder="Content">{{ old('content', $article->content) }}</textarea>
                            </div>
                            <label for="author" class="text-lg font-medium">Author</label>
                            <div class="my-3">
                                <input value="{{ old('author', $article->author) }}" type="text" id="author"
                                    name="author" class="form-input border-gray-300 shadow-sm w-1/2 rounded-lg"
                                    placeholder="Author">
                                @error('author')
                                    <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                                @enderror
